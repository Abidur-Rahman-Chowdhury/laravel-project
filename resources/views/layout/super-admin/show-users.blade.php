                  ...
                  if (!empty($data)) :
                    foreach ($data as $key => $value) { ?>
                      <tr>
                        ...
                        <td>
                          {{ $value->b_date == '0000-00-00' ? '' : $value->b_date }}
                        </td>
                        <td>
                          {{ $value->start_date == '0000-00-00' ? '' : $value->start_date }}
                        </td>
                        <td>
                          {{ $value->end_date  == '0000-00-00' ? '' : $value->end_date }}
                        </td>
                        <td>
                          {{ $value->created_at }}
                        </td>
                        ...
                        <td class="">
                            <div class="d-flex gap-2 justify-content-center">
                              <a href="/super-admin/edit-user/{{$value->id}}"><i style="font-size: 20px;" class='bx bxs-edit'></i></a>
                              <?php $actDec = $value->is_active == 0 ? 1  : 0  ?>
  ...
  function actDecData(routing, id, actDec, table, backRoute) {
    if (actDec == 0) {
      if (confirm('Do you want to deactivate?')) {
        $.ajax({
          url: 'super-admin/act-dec/' + routing + '/' + id + '/' + actDec + '/' + table + '/' + backRoute,
